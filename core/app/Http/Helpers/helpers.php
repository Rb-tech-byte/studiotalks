...
{
    // Provide a safe default to prevent boot-time errors if settings are missing
    $template = 'basic';
    try {
        if (\Illuminate\Support\Facades\Schema::hasTable('general_settings')) {
            $general = GeneralSetting::first(['active_template']);
            if ($general && $general->active_template) {
        ...
        // Fallback to default 'basic' without interrupting the boot process
    }
...
function getPageSections($arr = false)
{
    ...
    $sections = json_decode(file_get_contents($jsonUrl), $arr);
...
function showAdvertisement(){
    $advertisement = Advertisement::where('status', 1)->inRandomOrder()->first();

    if($advertisement){
        $advertisement->increment('impression');
        $advertisement->save();
        if ($advertisement->type == 1) {
            $html = '<div class="sidebar-widget mt-4">
                        <a href="'.route('adRedirect',encrypt($advertisement->id)).'"target="_blank" class="d-block">
                            <img src="'.getImage('assets/images/showAds/'.$advertisement->image).'"class="object-fit--cover">
                        </a>
                    </div>';
        }else{
            $html = '<div class="sidebar-widget ajaxCount mt-4" data-id="'.encrypt($advertisement->id).'">
                        aro jore'.$advertisement->script.'
                    </div>';
        }
        echo $html;
    }
...
function lastActivities($posts){
    $postsId = [];
    foreach($posts as $post){
        $postsId[] = $post->id;
    }

    $latestComment = Comment::whereIn('post_id', $postsId)->latest()->first();

    if($latestComment){
        return diffForHumans($latestComment->created_at);
    }
}
